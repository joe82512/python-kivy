...
class Login(Screen): 
    ...
    def app_login(self):
        ...
        self.build_checkpwd() #create page
        # demo_app.checkpwd.update_info() #update page
        demo_app.sm.current = 'Check' #change page

    def build_checkpwd(self): #solution2
        global screen_a
        checkpwd = CheckPwd()
        screen_a = Screen(name='Check')
        screen_a.add_widget(checkpwd)
        demo_app.sm.add_widget(screen_a)
...
class CheckPwd(Screen):
    ...
    def back_page(self):
        demo_app.sm.current = 'Login'
        demo_app.sm.remove_widget(screen_a) #destroy
...
class DemoApp(App):
    def build(self):
        ...
        screen = Screen(name='HW')
        screen.add_widget(self.helloworld)
        self.sm.add_widget(screen)
